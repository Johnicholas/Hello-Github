...
void make_binding(node_ptr l1, node_ptr l2, node_ptr* new_environ)
/*
...
*/
{
  ...
  case FUNC: fail(unify_return_value, new_environ, environ); break;
  case FREE_NODE: assert(0); break;
  }
}
/* unify_constant */
...
{
  switch (tag_value(head(list2))) {
  case CONSTANT: fail(unify_return_value, new_environ, environ); break;
  case VARIABLE: 
    var_ptr= look_up(string_val(head(list2)), environ);
    if (var_ptr == NULL) {
      make_binding(list2, list1, &environ);
  ...
  case FUNC:
    ...
    break;
  case CONS_NODE: fail(unify_return_value, new_environ, environ); break;
  case FREE_NODE: assert(0); break;
  }
}
/* unify_func */
