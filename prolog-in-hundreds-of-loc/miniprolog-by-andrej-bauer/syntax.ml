...
(** A conjunction of atomic propostions. The empty
...
(** [subst_term sub t] substitutes in term [t] values for variables,
    ...
    repeatedly until the term stops changing, so this is not the
...
| Var (v, 0) -> v
| Var (v, n) -> v ^ string_of_int n (** Johnicholas says: apparently ^ is ml for string concatenation *)
| Const c -> c
...
(** [string_of_env env] converts environment [env] to its string
    ...
    those that appear in the top level goal. *)
