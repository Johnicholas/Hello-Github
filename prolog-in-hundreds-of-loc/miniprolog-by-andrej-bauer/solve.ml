...
(** A value of type [choice] represents a choice point in the proof
    ...
    it is a tuple [(asrl, env, c, n)] where [asrl]
...
and solve ch asrl env c n =
    (** [reduce_atem a asrl] reduces atom [a] to subgoals by using the
...
(** [solve_toplevel c] searches for the proof of clause [c] using the
    global database [!base]. This function is called from the main
    program. *)
