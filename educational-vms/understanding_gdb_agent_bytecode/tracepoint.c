...
/* The agent expression evaluator, as specified by the GDB docs. It
   ...
   otherwise.  */
// the machine has a program counter
// the machine has a stack
// the machine has a top-of-stack register
// the stack holds "ulongest" values, ambiguously 1,2,4,8-byte unsigneds
// the machine has a stack pointer
//
// ops include:
//  add (two things on the stack)
//  subtract (two things on the stack)
//  multiply (two things on the stack)
//  signed divide (two things on the stack, checked)
//  unsigned divide (two things on the stack, checked)
//  signed remainder (checked)
//  unsigned remainder (checked)
//  shift left
//  signed shift right
//  unsigned shift irght
//  trace - this reads a length and offset from the traced code?
//  trace quick - this takes a literal argument, used for the length
//  not - logical negation
//  and - bitwise and
//  or - bitwise or
//  xor - bitwise xor
//  not - bitwise not
//  equal - binary compare equality
//  signed less - compare less than
//  unsigned less - compare less
//  ext - this takes a literal argument. some clever bit twiddling, what is the purpose?
//  ref8 - read one byte from memory, and put it on the stack
//  ref16 - read two bytes from memory, put it on the stack
//  ref32 - read 4 bytes
//  ref64 - read 8 bytes
//  if_goto - jump if true, to a 16-bit address that is in the bytecode
//  goto - unconditional jump to a 16-bit address that is in the bytecode
//  const8 - push a literal byte
//  const16 - push a literal two-byte value
//  const32 - push a literal four-byte value
//  const64 - push a literal eight-byte value
//  reg - a literal two-byte value points to a register, and gets the value of that register
//  end - halt and possibly return a value
//  dup - duplicate the top of the stack
//  pop - remove the top of the stack
//  pick - using a literal byte, pick something from deep in the stack and push it on the stack
//  rot - rotate the top three elements of the stack
//  zero_ext - some bit twiddling - what is the purpose?
//  swap - swap the top two stack values
//  getv - uses a two-byte literal argument to call "get_trace_state_variable_value" and push it
//  setv - uses a two-byte literal argument to call "set_trace_state_variable_value".. and push the new value?
//  tracev - uses a two-byte literal argument to call "agent_tsv_read"?

// ext and zero_ext are probably something to do with sign extension and zero extension
// which might be a common operation if you're doing this "ambiguous size" thing.
// the interesting ones are trace/ref which use "agent_mem_read"
//   (which uses "read_inferior_memory" internally)
// various reg which use "collect_register" (and supporting register_size and get_context_regcache)
// getv, setv, tracev which use "get/set_trace_state_variable_value" and "agent_tsv_read"
//  (which uses "get_trace_state_variable_value" internally)
